...
export const useDownloadDance = () => {
  ...
  const fetchDanceData = async (dance: Dance) => {
    ...
    const coverPromise = fetchWithProgress(toProxyUrl(dance.cover), {
      onProgress: (loaded, total) => {
        setCoverProgress(Math.ceil((loaded / total) * 100));
      },
    }).then(blobToDataURI);

    const audioPromise = await fetchWithProgress(toProxyUrl(dance.audio), {
      onProgress: (loaded, total) => {
        setAudioProgress(Math.ceil((loaded / total) * 100));
      },
    });

    const dancePromise = fetchWithProgress(toProxyUrl(dance.src), {
      onProgress: (loaded, total) => {
    ...
    const cameraPromise = dance.camera
      ? fetchWithProgress(toProxyUrl(dance.camera), {
          onProgress: (loaded, total) => {
